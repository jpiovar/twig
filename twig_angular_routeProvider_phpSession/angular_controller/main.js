...
app.controller("MainCtrl", ['$scope','$http','$location', function($scope,$http,$location) {
	...
	$scope.sesProcess = function(a,b){
		debugger;
		
		$http.post("dblib_exec/sesprocess.php",JSON.stringify({'sesok':a,'sesoperation':b}))
			.success(function(data, status){debugger;
				$scope.codeStatus = status;
				$scope.dataPost = data;
			})
...
app.controller("Temp1Ctrl", ['$scope','$http','$location', function($scope,$http,$location) {
	...
	_this.sesProcess = function(a,b){
		debugger;
		
		$http.post("dblib_exec/sesprocess.php",JSON.stringify({'sesok':a,'sesoperation':b}))
			.success(function(data, status){debugger;
				_this.codeStatus = status;
				_this.dataPost = data;
			})
			.error(function (data, status){debugger;
				_this.codeStatus = status.status || 'chyba';
			});
